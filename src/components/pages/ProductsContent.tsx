    ...
    <>
      ...
      <div className="bg-[#faf7f2]">
        {categories.map((cat, catIdx) => {
          ...
          return (
            <section key={cat.id} id={cat.id} className={catIdx % 2 === 0 ? "bg-[#faf7f2] py-20" : "bg-white py-20"}>
                      ...
                      return (
                        <div key={sub.id} id={sub.id}>
                          ...
                          <div className="flex items-center gap-3 mb-6">
                            <div className="w-1 h-8 rounded-full bg-[#0d5c63]" />
                              ...
                              <div className="flex items-center gap-3 flex-wrap">
                                ...
                                {sub.modelUrl && (
                                  <button
                                    ...
                                    className="flex items-center gap-1.5 px-3 py-1.5 bg-[#1a1a1a] text-white text-xs font-bold uppercase tracking-wider rounded-lg hover:bg-[#0d5c63] transition-colors"
                                  >
      ...
      <section className="bg-[#faf7f2] py-16 px-6">
        <div className="max-w-3xl mx-auto text-center">
          <div className="relative bg-gradient-to-br from-neutral-950 to-neutral-800 rounded-[2rem] p-10 lg:p-14 overflow-hidden shadow-elevated">
            <div className="absolute -top-16 -right-16 w-56 h-56 rounded-full bg-[#0d5c63]/25 blur-[70px]" />
            <div className="relative">
              ...
              <Link href="/contact" className="inline-block px-8 py-4 bg-[#0d5c63] text-white font-semibold rounded-full hover:bg-[#0a4a50] transition-all touch-manipulation shadow-[0_10px_30px_-8px_rgba(13,92,99,0.6)] hover:-translate-y-0.5 no-underline">
                {p.cta_btn}
              </Link>
            </div>
          </div>
      ...
      {lightbox && (
        <div className="fixed inset-0 bg-black/90 z-50 flex items-center justify-center p-4" onClick={() => setLightbox(null)}>
          <button className="absolute top-4 right-4 text-white hover:text-[#0d5c63] transition-colors z-10" onClick={() => setLightbox(null)}>
            <X size={32} />
          </button>
