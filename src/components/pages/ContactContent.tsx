            ...
            <div className="space-y-5 mb-10">
              {[
                ...
                <div key={item.label} className="flex gap-4 p-4 -mx-4 rounded-2xl hover:bg-white transition-colors group">
                  <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-[#e7f0f0] to-[#c9dcd9] flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform">
                    <item.icon size={20} className="text-[#0d5c63]" />
                  </div>
                  <div className="pt-0.5">
                    <p className="text-[11px] text-neutral-400 uppercase tracking-[0.15em] font-bold mb-1">{item.label}</p>
                    {item.href ? (
                      <a href={item.href} className="text-neutral-900 font-medium hover:text-[#0d5c63] transition-colors">{item.value}</a>
                    ) : (
