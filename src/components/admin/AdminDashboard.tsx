    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <header className="bg-[#1a1a1a] text-white px-4 md:px-6 py-4 flex items-center justify-between sticky top-0 z-40 shadow-lg">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-full bg-[#dc2626] flex items-center justify-center font-bold text-sm">T</div>
          <div>
      ...
      <div className="bg-white border-b border-gray-200 px-4 md:px-6 flex">
        <button onClick={() => setTab("images")} className={`flex items-center gap-2 px-5 py-4 text-sm font-semibold border-b-2 transition-colors ${tab === "images" ? "border-[#dc2626] text-[#1a1a1a]" : "border-transparent text-gray-400 hover:text-gray-600"}`}>
          <ImageIcon size={16} /> Fotot
        </button>
        <button onClick={() => setTab("products")} className={`flex items-center gap-2 px-5 py-4 text-sm font-semibold border-b-2 transition-colors ${tab === "products" ? "border-[#dc2626] text-[#1a1a1a]" : "border-transparent text-gray-400 hover:text-gray-600"}`}>
          <Package size={16} /> Produktet
        </button>
        <button onClick={() => setTab("faq")} className={`flex items-center gap-2 px-5 py-4 text-sm font-semibold border-b-2 transition-colors ${tab === "faq" ? "border-[#dc2626] text-[#1a1a1a]" : "border-transparent text-gray-400 hover:text-gray-600"}`}>
          FAQ
        </button>
        <button onClick={() => setTab("settings")} className={`flex items-center gap-2 px-5 py-4 text-sm font-semibold border-b-2 transition-colors ${tab === "settings" ? "border-[#dc2626] text-[#1a1a1a]" : "border-transparent text-gray-400 hover:text-gray-600"}`}>
          Cilësimet
        </button>
      </div>
        ...
        {tab === "images" && (
          <>
            ...
            <main className="flex-1 min-w-0">
              {/* Upload zone */}
              <div
                className={`mb-8 border-2 border-dashed rounded-2xl p-6 bg-white transition-colors ${dragOver ? "border-[#dc2626] bg-[#dc2626]/5" : "border-gray-200"}`}
              ...
              >
                <div className="mb-5">
                  ...
                  <select value={uploadSection} onChange={(e) => setUploadSection(e.target.value)}
                    className="w-full px-3 py-2.5 rounded-xl border border-gray-200 focus:border-[#dc2626] focus:outline-none text-sm text-gray-700">
                ...
                <div className="text-center">
                  ...
                  <p className="text-gray-500 text-sm mb-1">
                    Tërhiq & lësho, ose{" "}
                    <button onClick={() => fileRef.current?.click()} className="text-[#dc2626] font-semibold hover:underline">zgjidh skedarë</button>
                  </p>
                  ...
                  {uploading && <div className="mt-3 flex items-center justify-center gap-2 text-[#dc2626]"><RefreshCw size={16} className="animate-spin" /><span className="text-sm">Duke ngarkuar...</span></div>}
                </div>
              </div>
              ...
              <div className="flex items-center justify-between mb-4 gap-3 flex-wrap">
                <p className="text-sm font-semibold text-gray-600">
                  ...
                  {selectMode && selected.size > 0 && <span className="ml-2 text-[#dc2626]">· {selected.size} të zgjedhura</span>}
                </p>
                  ...
                  {filtered.map((img) => {
                    ...
                    <div
                      ...
                      className={`bg-white rounded-2xl overflow-hidden border transition-all cursor-grab active:cursor-grabbing ${
                        ...
                        isDragOver ? "border-[#dc2626] ring-2 ring-[#dc2626]/40 scale-[1.02]" :
                        isSelected ? "border-[#dc2626] ring-2 ring-[#dc2626]/30" :
                        !img.visible ? "opacity-50 border-gray-200" :
                        "border-gray-100 hover:border-[#dc2626]/50 hover:shadow-md"
                      }`}
                    >
                      <div className="relative aspect-square bg-gray-100">
                        ...
                        <div className="absolute inset-0 bg-black/0 hover:bg-black/50 transition-all flex items-center justify-center gap-2 opacity-0 hover:opacity-100">
                          <button onClick={() => toggleVisible(img)} className="w-9 h-9 rounded-full bg-white flex items-center justify-center hover:bg-[#dc2626] hover:text-white transition-colors">{img.visible ? <Eye size={15} /> : <EyeOff size={15} />}</button>
                        ...
                        {selectMode && (
                          <button
                            ...
                            className={`absolute top-2 left-2 w-6 h-6 rounded-md border-2 flex items-center justify-center transition-all ${isSelected ? "bg-[#dc2626] border-[#dc2626]" : "bg-white/80 border-gray-300 hover:border-[#dc2626]"}`}
                          >
                      ...
                      <div className="p-3">
                        {editingImgId === img.id ? (
                          <div className="space-y-2">
                            <select value={editImgSection} onChange={(e) => setEditImgSection(e.target.value)} className="w-full text-xs px-2 py-1.5 rounded-lg border border-gray-200 focus:border-[#dc2626] focus:outline-none">
                              {allSections.map((s) => <option key={s.value} value={s.value}>{s.label}</option>)}
                            </select>
                          ...
                          <>
                            <p className="text-xs text-[#dc2626] font-semibold truncate">{sectionLabel(img.section)}</p>
                    ...
                    <div key={cat.id} className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
                      ...
                      <div className="relative flex items-center justify-between px-6 py-4 bg-[#1a1a1a] overflow-hidden min-h-[72px]">
                        ...
                        <div className="relative flex items-center gap-3">
                          {cat.coverImage ? (
                            <div className="relative w-14 h-14 rounded-xl overflow-hidden ring-2 ring-[#dc2626]/60 shrink-0">
                        ...
                        <div className="relative flex items-center gap-2">
                          ...
                          <button
                            ...
                            className="flex items-center gap-2 px-4 py-2 bg-[#dc2626] text-white text-sm font-semibold rounded-xl hover:bg-[#b91c1c] transition-colors"
                          >
                      ...
                      {cat.coverImage && (() => {
                        ...
                        return (
                          <div className="px-6 py-3 bg-gray-50 border-b border-gray-100 flex items-center gap-4">
                            ...
                            <div
                              className="relative w-40 h-24 rounded-xl overflow-hidden border-2 border-[#dc2626]/40 cursor-crosshair shrink-0 hover:border-[#dc2626] transition-colors"
                              onClick={(e) => {
                            ...
                            >
                              ...
                              />
                              <div
                                className="absolute w-4 h-4 rounded-full bg-white border-2 border-[#dc2626] shadow-md -translate-x-1/2 -translate-y-1/2 pointer-events-none"
                      ...
                      {isAdding && (
                        <div className="px-6 py-5 bg-[#dc2626]/5 border-b border-[#dc2626]/20">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                            <div>
                              ...
                              <input value={newSub.name_sq} onChange={(e) => setNewSub((p) => ({ ...p, name_sq: e.target.value }))}
                                placeholder="p.sh. Tendë Tërheqëse" className="w-full px-3 py-2.5 rounded-xl border border-gray-200 focus:border-[#dc2626] focus:outline-none text-sm" />
                            </div>
                            <div>
                              ...
                              <input value={newSub.name_en} onChange={(e) => setNewSub((p) => ({ ...p, name_en: e.target.value }))}
                                placeholder="e.g. Retractable Awning" className="w-full px-3 py-2.5 rounded-xl border border-gray-200 focus:border-[#dc2626] focus:outline-none text-sm" />
                            </div>
                            <div>
                              ...
                              <textarea rows={2} value={newSub.desc_sq} onChange={(e) => setNewSub((p) => ({ ...p, desc_sq: e.target.value }))}
                                className="w-full px-3 py-2 rounded-xl border border-gray-200 focus:border-[#dc2626] focus:outline-none text-sm resize-none" />
                            </div>
                            <div>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                      <div>
                                        ...
                                        <input value={subEdits.name_sq ?? sub.name_sq} onChange={(e) => setSubEdits((p) => ({ ...p, name_sq: e.target.value }))}
                                          className="w-full px-3 py-2.5 rounded-xl border border-gray-200 focus:border-[#dc2626] focus:outline-none text-sm" />
                                      </div>
                                      <div>
                                        <label className="block text-xs text-gray-400 mb-1">🇬🇧 Emri (Anglisht)</label>
                                      ...
                                      <div>
                                        ...
                                        <input
                                          ...
                                          className="w-full px-3 py-2.5 rounded-xl border border-gray-200 focus:border-[#dc2626] focus:outline-none text-sm font-mono"
                                        />
                                      </div>
                                    </div>
                                    <div className="flex flex-wrap items-center gap-3">
                                  ...
                                  <div className="flex items-start justify-between gap-4">
                                    <div className="flex-1">
                                      <div className="flex items-center gap-3 mb-1">
                                        ...
                                        {imgCount > 0 && (
                                          <span className="text-xs bg-[#dc2626]/10 text-[#dc2626] font-semibold px-2 py-0.5 rounded-full">
                                            {imgCount} foto
                                          </span>
                                        )}
                                      </div>
                                    ...
                                    <div className="flex items-center gap-2 shrink-0">
                                      <button onClick={() => { setEditingSubId(sub.id); setSubEdits({}); }}
                                        className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-gray-600 border border-gray-200 rounded-lg hover:border-[#dc2626] transition-colors">
        ...
        <main className="flex-1 min-w-0">
          ...
          <div className="flex items-center justify-between mb-8">
            ...
            <button
              ...
              className={`flex items-center gap-2 px-5 py-2.5 text-sm font-semibold rounded-xl transition-colors ${addingFaq ? "bg-gray-100 text-gray-600 hover:bg-gray-200" : "bg-[#dc2626] text-white hover:bg-[#b91c1c]"}`}
            >
          ...
          {loadingFaqs ? (
            ...
            <div className="space-y-4">
              {/* ── Add form ── */}
              {addingFaq && (
                <div className="bg-white rounded-2xl border-2 border-[#dc2626]/25 shadow-sm overflow-hidden">
                  <div className="px-6 py-4 bg-gradient-to-r from-[#dc2626]/8 to-transparent border-b border-[#dc2626]/10 flex items-center gap-2">
                    <div className="w-6 h-6 rounded-full bg-[#dc2626] flex items-center justify-center shrink-0">
                      <Plus size={13} className="text-white" />
                    </div>
                    <p className="font-semibold text-[#dc2626] text-sm">Pyetje e re</p>
                  </div>
                      ...
                      <div className="space-y-4">
                        ...
                        <div>
                          ...
                          <input value={newFaq.q_sq} onChange={(e) => setNewFaq((p) => ({ ...p, q_sq: e.target.value }))}
                            ...
                            className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-[#dc2626] focus:outline-none text-sm bg-gray-50 focus:bg-white transition-colors" />
                        </div>
                        <div>
                          ...
                          <textarea rows={4} value={newFaq.a_sq} onChange={(e) => setNewFaq((p) => ({ ...p, a_sq: e.target.value }))}
                            ...
                            className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-[#dc2626] focus:outline-none text-sm resize-none bg-gray-50 focus:bg-white transition-colors" />
                        </div>
                      </div>
                      {/* English column */}
                      <div className="space-y-4">
                    ...
                    <div className="flex items-stretch">
                      ...
                      <div className="flex flex-col items-center justify-center gap-1.5 px-4 py-5 bg-gray-50 border-r border-gray-100 shrink-0">
                        ...
                        <div className="w-7 h-7 rounded-lg bg-[#dc2626]/10 flex items-center justify-center">
                          <span className="text-xs font-bold text-[#dc2626]">{idx + 1}</span>
                        </div>
                      ...
                      <div className="flex-1 min-w-0 px-6 py-5 flex items-start justify-between gap-4">
                        ...
                        <div className="flex items-center gap-2 shrink-0 pt-0.5">
                          <button onClick={() => { setEditingFaqId(faq.id); setFaqEdits({}); }}
                            className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-gray-600 bg-gray-100 rounded-lg hover:bg-[#dc2626]/10 hover:text-[#dc2626] transition-colors">
                            <Edit2 size={12} /> Ndrysho
                          </button>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            ...
            <div className="p-6 space-y-4">
              ...
              <button onClick={handleSaveHours} disabled={savingHours}
                className="w-full py-3 bg-[#dc2626] text-white text-sm font-bold rounded-xl hover:bg-[#b91c1c] transition-colors disabled:opacity-40 flex items-center justify-center gap-2">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="flex items-center justify-between p-4 bg-gray-50 rounded-xl border border-gray-100">
                ...
                <button
                  ...
                  className={`relative w-12 h-6 rounded-full transition-colors ${ann.active ? "bg-[#dc2626]" : "bg-gray-300"}`}
                >
              ...
              <div>
                ...
                <div className="flex gap-3">
                  {([["red", "bg-[#dc2626]", "E Kuqe"], ["dark", "bg-[#1a1a1a]", "E Zezë"], ["yellow", "bg-amber-400", "Amber"]] as const).map(([val, cls, label]) => (
                    <button key={val} onClick={() => setAnn((p) => ({ ...p, bg: val }))}
                      className={`flex items-center gap-2 px-4 py-2.5 rounded-xl border-2 text-sm font-semibold transition-all ${ann.bg === val ? "border-[#dc2626] scale-105" : "border-transparent bg-gray-100 text-gray-600 hover:border-gray-300"}`}>
              ...
              <div>
                ...
                <input value={ann.link ?? ""} onChange={(e) => setAnn((p) => ({ ...p, link: e.target.value || undefined }))}
                  ...
                  className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-[#dc2626] focus:outline-none text-sm bg-gray-50 focus:bg-white transition-colors font-mono" />
              </div>
              ...
              {(ann.text_sq || ann.text_en) && (
                <div className="rounded-xl overflow-hidden border border-gray-200">
                  ...
                  <div className={`py-2.5 px-4 text-center text-sm font-semibold relative ${ann.bg === "red" ? "bg-[#dc2626] text-white" : ann.bg === "dark" ? "bg-[#1a1a1a] text-white" : "bg-amber-400 text-[#1a1a1a]"}`}>
                    {ann.text_sq || ann.text_en}
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            ...
            <div className="p-6 space-y-5">
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    placeholder="••••••••"
                    className="w-full pl-4 pr-11 py-3 rounded-xl border border-gray-200 focus:border-[#dc2626] focus:outline-none text-sm bg-gray-50 focus:bg-white transition-colors"
                  />
                  ...
                  <input
                    ...
                    className={`w-full pl-4 pr-11 py-3 rounded-xl border focus:outline-none text-sm transition-colors ${
                      confirmPwd && confirmPwd !== newPwd
                        ...
                        : "border-gray-200 bg-gray-50 focus:border-[#dc2626] focus:bg-white"
                    }`}
                  />
              ...
              <button
                ...
                className="w-full py-3 bg-[#dc2626] text-white text-sm font-bold rounded-xl hover:bg-[#b91c1c] transition-colors disabled:opacity-40 flex items-center justify-center gap-2 mt-2"
              >
