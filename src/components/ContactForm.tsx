...
export default function ContactForm() {
  ...
  if (sent) {
    return (
      <div className="bg-white rounded-3xl p-12 border border-neutral-200 shadow-soft flex flex-col items-center justify-center text-center gap-4 min-h-[400px]">
        ...
        <button onClick={() => setSent(false)} className="mt-4 px-6 py-3 border border-neutral-200 rounded-full text-sm font-medium text-neutral-800 hover:border-[#a03e14] hover:text-[#a03e14] transition-colors touch-manipulation">
          {c.send_another}
        </button>
      </div>
    );
  ...
  const inputClass = "w-full px-4 py-3 rounded-xl border border-neutral-200 bg-white focus:border-[#a03e14] focus:ring-4 focus:ring-[#a03e14]/10 focus:outline-none text-neutral-900 text-sm transition-all placeholder:text-neutral-400";
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white rounded-3xl p-6 sm:p-8 border border-neutral-200 shadow-soft space-y-5">
      ...
      <button type="submit" disabled={loading} className="w-full py-4 bg-[#a03e14] text-white font-semibold rounded-xl hover:bg-[#7c2f0f] transition-all flex items-center justify-center gap-2 disabled:opacity-60 touch-manipulation shadow-[0_10px_24px_-8px_rgba(160,62,20,0.5)] hover:-translate-y-0.5">
        {loading ? (
